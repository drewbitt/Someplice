...
export const codeInput = {
	observedAttributes: ['value', 'placeholder', 'lang', 'template', 'onchange', 'onselectionchange'],
	...
	/* Plugins */
	plugins: {
		// Import a plugin from the plugins folder and it will be saved here.
	},
	Plugin: class {
		constructor() {
	...
	CodeInput: class extends HTMLElement {
		...
		sync_scroll() {
			...
			let result_element = this.template.preElementStyled
		...
		setup() {
			...
			textarea.setAttribute(
				'oninput',
				'this.parentElement.update(this.value); this.parentElement.sync_scroll();'
			);
		...
		if (codeInput.defaultTemplate === undefined) {
			...
			if (undefined in codeInput.templateQueue) {
				for (let i in codeInput.templateQueue[undefined]) {
					let elem = codeInput.templateQueue[undefined][i];
					elem.template = template;
					elem.setup();
				}
			}
	...
	templates: {
		...
		prism(prism, plugins = []) {
			// Dependency: Prism.js (https://prismjs.com/)
			return {
				includeCodeInputInHighlightFunc: false,
		...
		hljs(hljs, plugins = []) {
			// Dependency: Highlight.js (https://highlightjs.org/)
			return {
				includeCodeInputInHighlightFunc: false,
		...
		characterLimit() {
			return {
				highlight: function (result_element, code_input, plugins = []) {
					...
					let normal_characters = code_input.escape_html(
						code_input.value.slice(0, character_limit)
					);
					...
					if (overflow_characters.length > 0) {
						result_element.innerHTML += ` <mark class="overflow-msg">${
							code_input.getAttribute('data-overflow-msg') || '(Character limit reached)'
						}</mark>`;
					}
				},
			...
			return {
				highlight: function (result_element, code_input) {
					...
					for (let i = 0; i < sections.length; i++) {
						html_result.push(
							`<span style="color: ${
								code_input.template.rainbow_colors[i % code_input.template.rainbow_colors.length]
							}">${code_input.escape_html(sections[i])}</span>`
						);
					}
